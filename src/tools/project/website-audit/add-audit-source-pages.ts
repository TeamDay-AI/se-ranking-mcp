...
export class ProjectAddAuditSourcePages extends BaseTool {
    ...
    registerTool(server: McpServer): void {
        server.registerTool(
            this.toolName('addAuditSourcePages'),
            {
                title: 'Add Audit Source Pages',
                description:
                    'Project Tool: Uploads a custom list of pages to crawl for an audit. Submitted as a multipart text file (one URL per line). Automatically sets settings.source_file to 1 so the audit will actually use the list on the next run.',
                inputSchema: {
            ...
            async (params: { audit_id: number; urls: string[]; name?: string }) => {
                const filename = params.name ?? 'urls.txt';
                const content = params.urls.join('\n');
                const file = new File([content], filename, { type: 'text/plain' });
                return this.makePostRequest(
                    `/audit/${params.audit_id}/source-pages`,
                    {},
                    { file },
                );
            },
