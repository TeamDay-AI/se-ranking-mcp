...
app.use((req, _res, next) => {
    console.warn(`[Request] ${new Date().toISOString()} ${req.method} ${req.originalUrl}\nheaders:${JSON.stringify(req.headers)}\nbody: ${JSON.stringify(req.body)}\n\n`);
    next();
});
// catch all requests for MCP requests
app.all('/mcp', async (req, res) => {
    // Read per-request tokens from custom headers, falling back to env vars
    const dataToken = req.headers['x-data-api-token'] || DATA_API_TOKEN || '';
    const projectToken = req.headers['x-project-api-token'] || PROJECT_API_TOKEN || '';
    ...
    await tokenStore.run({ data: dataToken, project: projectToken }, async () => {
        ...
        try {
            ...
            console.error('MCP Server Error:', err, req.method, req.url, 'headers:', JSON.stringify(req.headers), 'body:', JSON.stringify(req.body));
            void cleanup().catch(console.error);
            if (!res.headersSent)
