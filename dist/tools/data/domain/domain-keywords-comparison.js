        ...
        server.registerTool(this.toolName('getDomainKeywordsComparison'), {
            ...
            inputSchema: {
                ...
                domain: z
                    ...
                    .describe('The primary domain for the analysis. For a keyword gap analysis (`diff=1`), this will be the domain that HAS the keywords. Required if `url` is not provided.')
                    .optional(),
                url: z
                    .string()
                    .url()
                    .optional()
                    .describe('The primary full URL for the analysis. If provided, `domain` is ignored. Required if `domain` is not provided.'),
                compare: z
                    .string()
                    .min(1, 'compare is required')
                    .describe('The secondary domain or full URL for comparison. Must match the type of the primary parameter (`domain` or `url`). For a keyword gap analysis (`diff=1`), this will be the entity that is MISSING the keywords.'),
                type: z
                    .enum(['organic', 'adv'])
                    .optional()
                    .default('organic')
                ...
                diff: z
                    ...
                    .optional()
                    .default(0)
                    .describe('Controls the comparison mode. `0` = Common keywords (intersection): keywords both domains rank for. `1` = Keyword gap (difference): keywords the `domain` ranks for, but `compare` does not.'),
                order_field: z
...
//# sourceMappingURL=domain-keywords-comparison.js.map